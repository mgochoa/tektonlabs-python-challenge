...
from tekton_challenge.schemas.product import Product, ProductCreate, ProductUpdate
...
@router.get("", status_code=HTTP_200_OK, response_model=List[Product])
@inject
def get_products(
...
@router.get("/{product_id}", status_code=HTTP_200_OK, response_model=Product)
@inject
@error_handler
...
@router.post("", status_code=HTTP_201_CREATED, response_model=Product)
@inject
