...
discount_repository_mock = mock.Mock(spec=ProductDiscountRepository)
...
def test_create_product(http_client):
    ...
    product_discount_mock = ProductDiscount(raw_discount=5209348)

    repository_mock.add.return_value = product_mock
    discount_repository_mock.get_discount.return_value = product_discount_mock
    ...
    app.container.products_repository.override(repository_mock)
    app.container.cache_repository.override(cache_repository_mock)
    app.container.product_discount_repository.override(discount_repository_mock)

    response = http_client.post("/inventory/products", json=body)

    assert response.status_code == HTTP_201_CREATED
    assert response.json().get("product_id") == 1
    assert response.json().get("discount") == product_discount_mock.value
